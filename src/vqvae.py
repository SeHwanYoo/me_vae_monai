...
class VQVAE(nn.Module):
    ...
    def __init__(
        ...
        # Mean and log_var layers for the multi-encoder
        self.fc_mean = nn.Linear(embedding_dim, embedding_dim)
        self.fc_log_var = nn.Linear(embedding_dim, embedding_dim)
        
        # Mean and log_var layers for the multi-encoder
        self.fc_mean2 = nn.Linear(embedding_dim, embedding_dim)
        self.fc_log_var2 = nn.Linear(embedding_dim, embedding_dim)
    ...
    def encode_stage_2_inputs(self, x: torch.Tensor, quantized: bool = True) -> torch.Tensor:
        ...
        # z2 = self.encode2(x)
        # z2_mean = self.fc_mean2(z)
        # z2_log_var = self.fc_log_var2(z)
